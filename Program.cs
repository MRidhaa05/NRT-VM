...
namespace NRTVending
{
    class Program
    {
            ...
            while (true)
            {
                ...
                switch (choice)
                {
                    ...
                    case "Exit":
                        ...
                        Console.ResetColor();
                        Thread.Sleep(5000);
                        Console.WriteLine("\nYou may now exit the console by pressing any key...");
                        Console.ReadKey();
                        return;
                }
            }
            ...
            while (true)
            {
                ...
                switch (musicchoice)
                {
                    ...
                    case "8-BIT Console Music":
                        AnsiConsole.MarkupLine("[green1]Playing '8-BIT'[/]");
            ...
            while (adminattempts < maxAttempts)
            {
                ...
                if (admin != null)
                {
                    while (true)
                    {
                        ...
                        AnsiConsole.MarkupLine($"Admin Cashout Balance: [green1]R{admin.Balance:0.00} [/]\n");
            ...
            while (loginAttempts < maxAttempts)
            {
                ...
                while (true)
                {
                    ...
                    if (string.IsNullOrEmpty(studentId) || studentId.Length != 8)
                    {
                        ...
                        AnsiConsole.MarkupLine("[red1]Only registered users can log in from the table above[/]");
                        continue;
                    }
                    break;
                }
                ...
                {
                    loginAttempts++;
                    AnsiConsole.MarkupLine($"[red1]Invalid credentials! Attempts remaining: {maxAttempts - loginAttempts}[/]");
            ...
            while (true)
            {
                ...
                switch (choice)
                {
                    ...
                    case "Withdraw Specific Amount":
                        var amountPrompt = new TextPrompt<decimal>("[orange1]Enter withdraw amount:[/]")
                            .Validate(a => a > 0 ? ValidationResult.Success() : ValidationResult.Error("[red1]Amount must be greater than 0![/]"));
        ...
        static void Purchase(User user)
        {
            while (true)
            {
                var idPrompt = new TextPrompt<int>("Enter [orange1]Item ID[/] to purchase:")
        ...
        static void UpdateItemStock()
        {
            while (true)
            {
                int id = AnsiConsole.Prompt(new TextPrompt<int>("Enter [red1]Item ID[/] to update stock:")
                    .Validate(i => i >= 0 ? ValidationResult.Success() : ValidationResult.Error("[red]Invalid ID[/]")));
